...
# 1 = djed pillar, 2 = shen ring. Built in the XY plane facing +Z;
# `size` is the glyph height.
...
static func build(kind: int, size: float, emissive: bool = true) -> Node3D:
	...
	match kind:
		...
		_:  # Shen ring: circle resting on a bar.
			_ring(root, gold, 0.27 * s, 0.07 * s, Vector3(0, 0.1 * s, 0))
			_bar(root, gold, Vector3(0.62 * s, 0.1 * s, 0.08 * s), Vector3(0, -0.4 * s, 0))
	return root


static func _bar(root: Node3D, material: Material, size: Vector3, pos: Vector3) -> void:
